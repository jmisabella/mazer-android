package com.jmisabella.mazeq.layout
...
fun computeCellSize(mazeCells: List<MazeCell>, mazeType: MazeType, cellSize: CellSize, context: Context): Float {
    ...
    return when (mazeType) {
        MazeType.ORTHOGONAL -> screenWidth / cols.toFloat()
        MazeType.DELTA -> computeDeltaCellSize(cellSize, cols, screenWidth, displayMetrics.heightPixels.toFloat())
        MazeType.SIGMA -> {
            val units = 1.5f * (cols - 1).toFloat() + 1f
            screenWidth / units
        }
        else -> screenWidth / cols.toFloat()
    }
}

fun adjustedCellSize(mazeType: MazeType, cellSize: CellSize, context: Context): Float {
    val displayMetrics = context.resources.displayMetrics
    val screenSize = displayMetrics.widthPixels.toFloat() to displayMetrics.heightPixels.toFloat()
...
fun computeDeltaCellSize(
    ...
    for (entry in filteredMap) {
        val distance = abs(screenWidth - entry.first.first) + abs(screenHeight - entry.first.second)
...
fun navigationMenuVerticalAdjustment(mazeType: MazeType, cellSize: CellSize, context: Context): Float {
    val displayMetrics = context.resources.displayMetrics
    val screenSize = Pair(displayMetrics.widthPixels.toFloat(), displayMetrics.heightPixels.toFloat())
